...
NPS has developed this template for preparing NPS reports and theses. 
...
Text editors offer a variety of features.
...
\begin{itemize}
  ...
  re-wrap your text as necessary to format the paragraphs. Indention 
  is determined by the style of the current document, not by whether 
  or not you actually indent the text in your input file.
\item \LaTeX{} ignores space at the beginning of lines; breaks between
  paragraphs are marked using blank lines.
\item What you type is not what you get!  In particular, opening
  ...
  the end of a sentence, so it can place whitespace correctly.
...
\item Comments can be embedded in your document with a percent sign
  (|%|). Anything after the percent sign will not print. To actually print a
...
With \LaTeX{} most accented vowls are entered with a combining accent character
...
\raisebox{-3pt}{\includegraphics[height=12pt]{images/xelatex}} (|xelatex|), or
by saving your Unicode characters in a |.pdf| file and including that file with
the |\includegraphics{}| command (as we did with the
\raisebox{-3pt}{\includegraphics[height=12pt]{images/xelatex}} logo).
...
This section briefly describes how to embed graphics in a \LaTeX{} document.
For alternative treatments we recommend H\"oppner's  ``Strategies for including
graphics in \LaTeX{} documents''\cite{strategies}, and the book \emph{The LaTeX
Graphics Companion}\cite{graphics-companion}.
...
Typical options that you can use are |width=XX| or |height=XX|. For example, to
include an image, have it centered, and scale it to 3 inches, use this:
